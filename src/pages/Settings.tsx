    ...
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="card">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="font-medium text-gray-900">Resumen diario con IA</p>
            <p className="text-sm text-gray-500">
              Genera un resumen motivacional basado en tus estadisticas del dia
            </p>
          </div>
          <button
            onClick={() =>
              updateSettings({
                aiCoachEnabled: !(settings.aiCoachEnabled !== false),
              })
            }
            className={cn(
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out',
              settings.aiCoachEnabled !== false ? 'bg-primary-500' : 'bg-gray-200'
            )}
          >
            <span
              className={cn(
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                settings.aiCoachEnabled !== false ? 'translate-x-5' : 'translate-x-0'
              )}
            />
          </button>
        </div>
